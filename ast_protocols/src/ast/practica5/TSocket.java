...
public class TSocket {
    ...
    protected Condition c_ack;
    protected Condition c_receive;
    ...
    protected TSocket(Protocol p, int localPort, int remotePort) {
        ...
        c_ack = lk.newCondition();
        c_receive = lk.newCondition();
        proto = p;
    ...
    public void sendData(byte[] data, int offset, int length) throws InterruptedException {
        ...
        try {
            ...
            int revUnc = offset;
            int segmsize;
            TCPSegment t = new TCPSegment();
            while (sndIsUna) {
                c_ack.await();
            }
            while (revUnc < length) {
                if ((length - revUnc) < sndMSS) {
                    segmsize = length - revUnc;
                } else {
                    segmsize = sndMSS;
                }
                t = segmentize(data, revUnc, segmsize);
                sendSegment(t);
                sndIsUna = true;
                revUnc = revUnc + segmsize;
            }

            //TODO:l’emissor, despŕes d’enviar un segment de dades, espera 
            //rebre el segment de reconeixement corresponent.
    ...
    public int receiveData(byte[] buf, int offset, int maxlen) throws InterruptedException {
        ...
        try {
            ...
            // A completar per l'estudiant:
            //TODO: COMPROBAR QUE ES CORRECTO. No sé...
            //¿Debería esperar y hacerse con while y await?
            int llegits = 0;
            while (rcvSegment == null) {
                c_receive.await();
            }
            while(offset + llegits < maxlen){
                processReceivedSegment(rcvSegment);
                llegits += consumeSegment(buf, offset+llegits, maxlen);
            }
