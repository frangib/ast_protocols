...
public class TSocket {
    ...
    protected Lock lk;
    protected Condition appCV, esperarAck;
    ...
    public void sendData(byte[] data, int offset, int count) throws InterruptedException {
        ...
        try {
            ...
            while(revUnc<count){
                if(((count-revUnc)>=sndMSS)) {
                    segmsize=sndMSS;
                }else{
                    segmsize=count-revUnc;
                }
                
                sndUnackedSegment=segmentize(data, revUnc,segmsize);
                sndUnackedSegment.setSeqNum(sndNxt);
                sndUnackedSegment.setPorts(localPort, remotePort);
    ...
    public int receiveData(byte[] buf, int offset, int maxcount) throws InterruptedException {
        ...
        try {
            ...
            while(llegits<maxcount) {
                llegits=consumeSegment(buf, offset+llegits, maxcount-llegits);
            }
